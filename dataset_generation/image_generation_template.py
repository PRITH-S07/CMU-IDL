...
def generate_image(
    prompt,
):  ##TO DO -> create function for your API which takes in prompt and returns requests response
    response = None
    return response
...
def generate_and_save_images(
    image_prompt_dict, generator="firefly", output_dir="generated_images"
):
    ...
    for image, prompt in image_prompt_dict.items():
        ...
        if response.status_code == 200:
            ...
            print(
                f"smh try again. image file: {image}. status code: {response.status_code}"
            )
